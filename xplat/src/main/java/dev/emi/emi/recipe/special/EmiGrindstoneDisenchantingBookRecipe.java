...
public class EmiGrindstoneDisenchantingBookRecipe implements EmiRecipe {
	...
	public EmiGrindstoneDisenchantingBookRecipe(Holder<Enchantment> enchantment, int level, Identifier id) {
	...
	private EmiStack getBook() {
		...
		enchBuilder.set(enchantment, level);
	...
	private FormattedCharSequence getExp() {
		int minPower = enchantment.value().getMinCost(level);
