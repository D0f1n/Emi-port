...
public class EmiRecipeFiller {
	...
	public static <T extends AbstractContainerMenu> List<EmiRecipeHandler<T>> getAllHandlers(AbstractContainerScreen<T> screen) {
		if (screen != null) {
			...
			MenuType<?> type;
			try {
				type = menu instanceof InventoryMenu ? null : menu.getType();
	...
	public static <T extends AbstractContainerMenu> @Nullable List<ItemStack> getStacks(StandardRecipeHandler<T> handler, EmiRecipe recipe, AbstractContainerScreen<T> screen, int amount) {
		try {
			...
			if (handler != null) {
				...
				List<Slot> craftingSlots = handler.getCraftingSlots(recipe, menu);
	...
	public static <T extends AbstractContainerMenu> boolean clientFill(StandardRecipeHandler<T> handler, EmiRecipe recipe,
		...
		T menu = screen.getMenu();
		if (handler != null && menu.getCarried().isEmpty()) {
			Minecraft client = Minecraft.getInstance();
			MultiPlayerGameMode manager = client.gameMode;
			Player player = client.player;
			List<Slot> clear = handler.getCraftingSlots(menu);
			for (Slot slot : clear) {
				if (slot != null) {
					manager.handleContainerInput(menu.containerId, slot.index, 0, ContainerInput.QUICK_MOVE, player);
				}
			}
			List<Slot> inputs = handler.getInputSources(menu);
			List<Slot> slots = handler.getCraftingSlots(recipe, menu);
			outer:
			for (int i = 0; i < stacks.size(); i++) {
				...
				for (Slot input : inputs) {
					...
					if (ItemStack.isSameItemSameComponents(is, stack)) {
						manager.handleContainerInput(menu.containerId, input.index, 0, ContainerInput.PICKUP, player);
						if (is.getCount() <= needed) {
							needed -= is.getCount();
							manager.handleContainerInput(menu.containerId, crafting.index, 0, ContainerInput.PICKUP, player);
						} else {
							while (needed > 0) {
								manager.handleContainerInput(menu.containerId, crafting.index, 1, ContainerInput.PICKUP, player);
								needed--;
							}
							manager.handleContainerInput(menu.containerId, input.index, 0, ContainerInput.PICKUP, player);
						}
			...
			Slot slot = handler.getOutputSlot(menu);
			if (slot != null) {
				if (destination == EmiCraftContext.Destination.CURSOR) {
					manager.handleContainerInput(menu.containerId, slot.index, 0, ContainerInput.PICKUP, player);
