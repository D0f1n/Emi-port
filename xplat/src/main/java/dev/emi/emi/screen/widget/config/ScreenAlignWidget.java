...
public class ScreenAlignWidget extends ConfigEntryWidget {
	...
	public void update(int y, int x, int width, int height) {
		horizontal.setMessage(getHorizontalText());
		vertical.setMessage(getVerticalText());
		horizontal.setX(x + width - horizontal.getWidth() - vertical.getWidth() - 7);
		horizontal.setY(y);
		vertical.setX(x + width - vertical.getWidth());
		vertical.setY(y);
	}
}
