...
/**
 * Serializes wrapped JEI ingredients by their JEI uid. JEI 30.x dropped the direct
 * uid-to-ingredient lookup, so deserialization scans the registered ingredients of custom types.
 */
...
public class JemiStackSerializer implements EmiIngredientSerializer<JemiStack> {
	...
	private EmiStack create(String uid, long amount) {
		for (IIngredientType<?> type : manager.getRegisteredIngredientTypes()) {
			...
			for (Object ingredient : manager.getAllIngredients(type)) {
