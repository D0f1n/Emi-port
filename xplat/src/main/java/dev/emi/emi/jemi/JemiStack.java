...
/**
 * An EMI stack wrapping a custom (non item/fluid) JEI ingredient type, rendered and described
 * entirely through JEI's helper/renderer pair.
 */
public class JemiStack<T> extends EmiStack {
	...
	public void render(GuiGraphicsExtractor raw, int x, int y, float delta, int flags) {
		...
		context.push();
		context.pose().translate(x + xOff, y + yOff);
	...
	public List<Component> getTooltipText() {
		return renderer.getTooltip(ingredient, TooltipFlag.Default.NORMAL);
	}
	...
	public List<ClientTooltipComponent> getTooltip() {
		...
		renderer.getTooltip(builder, ingredient, client.options.advancedItemTooltips ? TooltipFlag.Default.ADVANCED : TooltipFlag.Default.NORMAL);
