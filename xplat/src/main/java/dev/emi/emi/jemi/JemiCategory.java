...
/**
 * Wraps a JEI recipe category as an EMI one. The icon prefers JEI's own drawable, then the first
 * workstation, then a two-letter shorthand of the title.
 */
		...
		this.icon = (raw, x, y, delta) -> {
			...
			if (icon != null) {
				...
				if (!workstations.isEmpty()) {
					...
					context.drawCenteredTextWithShadow(EmiPort.literal(title.substring(0, Math.min(2, title.length()))), x + 8, y + 2, -1);
				}
			}
		};
